...
// the web enumerator finds nothing. Compare enumerates the SAME canonical runs
// capture writes (mobileCaseRuns, lib/mobile-capture-engine.mjs) so comparison
// identities always match the artifacts on disk — including the device matrix
// fan-out. The compare item keeps its historical shape: the raw display key
// ('label__viewport__state') plus routeName/viewportName/stateName for
// artifactPaths; masks/regions pass through from the case.
function enumerateMobileForCompare(config, filters = {}) {
  return mobileCaseRuns(config, filters).map((run) => ({
    key: run.key,
    routeName: run.identity.routeName,
    viewportName: run.identity.viewportName,
    stateName: run.identity.stateName,
    masks: Array.isArray(run.case.masks) ? run.case.masks : [],
    regions: Array.isArray(run.case.regions) ? run.case.regions : []
  }));
}
...
export async function compareAll(config, { filters = {} } = {}) {
  ...
  const items = isMobile
    ? enumerateMobileForCompare(config, filters)
