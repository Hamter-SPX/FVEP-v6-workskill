...
/**
 * Flow layer binding: each mode names the flow doc that governs its conversation-level
 * discipline, plus companion flows. The map lives in flow/flow-map.json — that file is
 * the single source of truth, guarded by tests/unit/flow-docs.test.mjs so every entry
 * points at a real doc. When the flow layer is absent the engine still resolves modes;
 * the flow fields simply report null/[].
 */
let FLOW_MAP = {};
try {
  FLOW_MAP = JSON.parse(fs.readFileSync(new URL('../flow/flow-map.json', import.meta.url), 'utf8'));
} catch {
  FLOW_MAP = {};
}

/** Flow doc and companions bound to a mode id, or null/[] when unmapped. */
export function flowForMode(id) {
  const key = String(id ?? '').trim().toLowerCase();
  const entry = FLOW_MAP[key];
  return {
    flow: typeof entry?.flow === 'string' ? entry.flow : null,
    flowCompanions: [...(entry?.companions ?? [])]
  };
}
...
export function resolveMode(requestText, options = {}) {
  ...
  const resolvedId = noSignal ? fallback : top.id;
  const { flow, flowCompanions } = flowForMode(resolvedId);
...
export function auditModeExit(input = {}, policy = {}) {
  ...
  const { flow, flowCompanions } = flowForMode(target.id);
  return {
    ...audit,
    mode: target.id,
...
export function formatModeCard(target) {
  ...
  const { flow, flowCompanions } = flowForMode(definition.id);
  if (flow) {
    lines.push(...bullets('FLOW', [
      `${flow} (governing)`,
      ...flowCompanions.map((doc) => `${doc} (companion)`)
    ]));
  }
