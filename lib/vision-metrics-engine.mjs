...
export function occupancyGrid(image, { cols = 8, rows = 5, tolerance = 48, emptyBelow = 0.02 } = {}) {
  ...
  // Reachability note: a minRatio >= 0.9 trigger can never fire with border-sampled
  // backgrounds (border pixels always match the guess, so edge cells sit far below
  // 0.9); "all cells full" is therefore detected directly via emptyCells — same
  // rule, reachable form. minRatio kept as a fallback for exotic grids/tolerances.
