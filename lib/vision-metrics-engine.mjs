...
export function occupancyGrid(image, { cols = 8, rows = 5, tolerance = 48, emptyBelow = 0.02 } = {}) {
  ...
  // Suspect background guess = fragmented border (low dominant share) or every cell
  // occupied (gradient border / modal ring makes the guessed background "wrong" —
  // downstream judges must not trust occupancy metrics when this flag is set).
  // "Every cell occupied" has two detectors:
  // - emptyCells.length === 0 (every cell above emptyBelow): the effective detector
  //   at the default config; the only reachable one at small fixture scale (80x40,
  //   10x8 cells) where the 4px sampled border dominates edge cells, capping their
  //   ratio near 0.5 so minRatio >= 0.9 cannot fire there.
  // - minRatio >= 0.9 (flat >=90% occupancy in every cell, independent of
  //   emptyBelow): covers exotic emptyBelow > 0.9 configs; also reachable at device
  //   scale, where a 4px border is a negligible cell fraction (probe: 1170x2532
  //   uniform border + full content -> minRatio ~= 0.95).
...
export function alignmentScore(image, { threshold = 0.18 } = {}) {
  ...
  // Bin key uses floor (not round) 2px buckets: rounding pulls near-half values
  // (e.g. spacing 5 and 6) into one bucket, which scored irregular noise as 0.5
  // consistent — see tests/unit/vision-metrics-engine.test.mjs scattered case.
