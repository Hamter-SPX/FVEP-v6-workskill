...
async function listKeys(directory, suffix) {
  try {
    ...
    return entries.filter((entry) => entry.isFile() && entry.name.endsWith(suffix)).map((entry) => entry.name.slice(0, -suffix.length));
  } catch { return []; }
}
...
export async function collectEvidence(outputDir) {
  ...
  for (const key of [...caseKeys].sort()) {
    ...
    const currentBytes = hasCurrent ? await fs.readFile(path.join(root, 'current', `${key}.png`)) : null;
    const referenceBytes = hasReference ? await fs.readFile(path.join(root, 'reference', `${key}.png`)) : null;
    const diffBytes = hasDiff ? await fs.readFile(path.join(root, 'diff', `${key}.png`)) : null;

    const meta = metadata.capture.current.get(key) ?? metadata.capture.reference.get(key) ?? null;
    ...
    const captureSha256 = typeof meta?.screenshotSha256 === 'string' ? meta.screenshotSha256 : metricsSha256;
    ...
    const verdict = judgment?.verdict ?? mapVerdict(severity);
    ...
    const captureMetaSuffix = metadata.capture.current.has(key) ? CURRENT_CAPTURE_SUFFIX : REFERENCE_CAPTURE_SUFFIX;
    cases.push({
      key,
      ...
      paths: {
        ...
        ...(meta ? { captureMetadata: `metadata/${key}${captureMetaSuffix}` } : {}),
...
function chip(verdict) {
  ...
  return `<span class="chip chip--${kind}">${escapeHtml(kind.toUpperCase())}</span>`;
}
...
function hashesBlock(hashes) {
  const capture = hashes?.captureSha256 ?? null;
  const metrics = hashes?.metricsSha256 ?? null;
  if (!capture && !metrics) return '<p class="muted">No hashes recorded.</p>';
  const stale = capture && metrics && capture !== metrics;
  const rows = [
    capture ? `<div class="hash-row">captureSha256 <code class="hash">${escapeHtml(capture)}</code></div>` : '',
    metrics ? `<div class="hash-row">metricsSha256 <code class="hash">${escapeHtml(metrics)}</code></div>` : '',
    stale ? '<div class="hash-row"><span class="stale">capture and metrics hashes diverge — the screenshot changed after it was judged; verdicts above describe different bytes. Re-run capture before trusting this card.</span></div>' : ''
  ].filter(Boolean).join('\n');
  return `<div class="hashes">${rows}</div>`;
}
