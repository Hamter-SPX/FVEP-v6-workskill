...
// compare-engine writes mobile comparisons[].key as the RAW `${label}__mobile__${key}`
// (enumerateMobileCasesForCompare), while artifactPaths keys are safeSegment-
// normalized — 'Home Page'/'home_screen' would otherwise fracture one case
// into a joined card plus a phantom. Normalizing the comparison key with the
// identical rule joins them; already-canonical segments pass through
// bit-identical. (Web comparison keys are artifactKey-canonical already.)
const MOBILE_MARKER = '__mobile__';
function normalizeComparisonKey(key) {
  const index = String(key).indexOf(MOBILE_MARKER);
  if (index === -1) return key;
  return `${safeSegment(String(key).slice(0, index))}${MOBILE_MARKER}${safeSegment(String(key).slice(index + MOBILE_MARKER.length))}`;
}
  ...
  for (const key of [...caseKeys].sort()) {
    ...
    const captureSha256 = typeof currentMeta?.screenshotSha256 === 'string'
      ...
      : metaSource === 'reference'
        ? null // capture-time hash of the current PNG unknown — do not fake it
        : metricsSha256;
