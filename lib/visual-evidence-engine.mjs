...
import { FALLBACK_RULE, lookupRemediationRule } from './remediation-rules.mjs';
...
// Findings whose rule maps to a curated remediation rule carry that rule's
// action as `fix`; the fallback stays silent, so an unmapped rule never gets
// generic guidance dressed up as case-specific advice.
function attachFixes(findings) {
  return (findings ?? []).map((finding) => {
    const rule = lookupRemediationRule(finding?.rule);
    return rule === FALLBACK_RULE ? finding : { ...finding, fix: rule.action };
  });
}
...
export async function collectEvidence(outputDir) {
  ...
  for (const key of [...caseKeys].sort()) {
    ...
    const findings = attachFixes(Array.isArray(judgment?.findings)
      ? judgment.findings
...
function findingsList(findings) {
  ...
  const fixRow = (finding) => (finding?.fix ? `<span class="fix">fix: ${escapeHtml(finding.fix)}</span>` : '');
  const rows = findings.map((finding) => `<li class="finding finding--${escapeHtml(finding?.severity ?? 'info')}"><code>${formatDynamic(finding?.rule)}</code> <span class="sev">${formatDynamic(finding?.severity)}</span> — expected ${formatJson(finding?.expected)} · observed ${formatJson(finding?.observed)}${fixRow(finding)}</li>`).join('\n');
...
.findings .fix{display:block;color:var(--muted);font-size:12px;margin-top:2px}
.hashes{margin:8px 0}
