...
// Failure-class severities for the insight layer. Insight families that narrate
// failure (streaks/regressions/resolved) count ONLY these: a mobile-check verdict
// of 'warn' never fails the gate (vision-loop counts fail verdicts), so warn
// findings must not be narrated as "ล้ม" either. Warn rows still stay recorded
// and feed the severity-blind families (recurring/correlations).
const FAILURE_SEVERITIES = new Set(['fail', 'blocker', 'major']);
...
export async function analyzeRunIntel(outputDir, { windowDays = 14, minOccurrences = 2, streak = 2, limitRuns = 200, store: injected } = {}) {
  ...
  try {
    ...
    // Failure-class view of the same rows — the failure-narrating families
    // (streaks/regressions/resolved) read this; warn rows are invisible here.
    const failByRun = new Map([...byRun.entries()].map(([runId, rows]) => [
      runId,
      rows.filter((row) => FAILURE_SEVERITIES.has(row.severity)),
    ]));
    ...
    // Adjacency helper: does failure-class (rule, case_key, device) appear in run at index i?
    const hasTriple = (index, triple) =>
      (failByRun.get(runs[index].run_id) ?? []).some((row) => dedupeKey(row) === triple);

    // (2) Streaks: failure-class (rule, case, device) present in every run from
    // the newest backwards. Warn findings neither seed nor extend a streak — a
    // warn never fails the gate, so counting it would narrate a pass-level run
    // as "ล้มติดกัน".
    const streaks = [];
    if (runs.length > 0) {
      const newestTriples = new Map();
      for (const row of failByRun.get(runs[0].run_id)) {
        const key = dedupeKey(row);
    ...
    // recurring — resolvedAfterRuns = index of its newest FAILURE-class
    // occurrence. Warn-only rules cannot be "resolved": they never failed, so
    // this family reads failByRun exclusively.
    // Absence-as-resolution is list-presence-based (same proxy family as the
    // absence proxy in regressions below): runs recorded with --case/--route
    // filters under-represent absence — not a guarantee the issue was fixed.
    // The caveat is repeated in the user-facing note because `npm run intel`
    // prints it verbatim.
    const failRuleRuns = new Map(); // rule → Map(runId → runPos), failure-class rows only
    for (const [runId, pos] of runPos) {
      for (const row of failByRun.get(runId)) {
        if (row.rule == null) continue;
        if (!failRuleRuns.has(row.rule)) failRuleRuns.set(row.rule, new Map());
        failRuleRuns.get(row.rule).set(runId, pos);
      }
    }
    const resolved = [];
    for (const [rule, failRuns] of failRuleRuns) {
      if (failRuns.size < minOccurrences) continue;
      const newestPos = Math.min(...failRuns.values());
      ...
      resolved.push({
        ...
        note: `rule '${rule}' ไม่พบใน ${newestPos} รันล่าสุด (fail ล่าสุด run ${lastFailureRunId}; ยึดตามการไม่พบ finding — รันแบบ filter ด้วย --case/--route อาจไม่ได้ตรวจ rule นี้จริง)`,
      });
    }
    resolved.sort((a, b) => b.resolvedAfterRuns - a.resolvedAfterRuns);

    // (5) Regressions: per case, the most recent adjacent absence→presence
    // transition over failure-class rows only. "Absence" means the run recorded
    // no failure-class finding for that case — a proxy, NOT a pass: a run
    // filtered by --case/--route may never have exercised the case (see the
    // absence-based wording this family renders as in scripts/intel.mjs).
    const caseKeys = new Set();
    for (const rows of failByRun.values()) {
      for (const row of rows) {
        if (row.case_key != null) caseKeys.add(row.case_key);
      }
    }
    const regressions = [];
    for (const caseKey of caseKeys) {
      const failsAt = (index) => (failByRun.get(runs[index].run_id) ?? []).some((row) => row.case_key === caseKey);
