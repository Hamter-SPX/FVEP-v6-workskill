...
// A device matrix must be an array of rows. Any other non-null value (e.g. the
// bare string "iphone16") used to silently degrade to [], which the fan-out
// engines read as "every device" — fail loudly with the offending value so the
// config author sees the row shape instead.
function normalizeMobileDevices(devices) {
  if (devices == null) return [];
  if (!Array.isArray(devices)) {
    throw new TypeError(`mobile.devices must be an array of device rows when present; got ${JSON.stringify(devices)} (use [{ key: "..." }] — a bare value would silently fan out to every device).`);
  }
...
export function normalizeConfig(input = {}, configPath = path.resolve('vision-loop.config.json')) {
  ...
  return {
    ...
    mobile: {
      ...
      // resolves the effective driver per run). A non-array value throws in
      // normalizeMobileDevices instead of degrading to [].
      devices: normalizeMobileDevices(source.mobile?.devices),
...
// The '__' identity separator is reserved in mobile identities: a comparison
// key joins label__device__case with it and normalizeComparisonKey
// (lib/visual-evidence-engine.mjs) splits evidence back apart on the same
// separator. A '__' inside any segment yields ≥4 parts and silently bypasses
// that join — the real card loses its verdict while a phantom fail card
// appears. Resolved values are checked (a case label defaults to the case key
// when absent), so the reservation covers the identity exactly as shipped.
function rejectIdentitySeparator(field, value) {
  if (!String(value).includes('__')) return;
  throw new TypeError(
    `Mobile ${field} "${value}" must not contain "__" — it is the identity separator of the joined ` +
    `comparison key (label__device__case) and extra segments silently bypass evidence joins; use a different character (e.g. "-").`
  );
}
...
export function validateConfig(config) {
  ...
  for (const mobileCase of config.mobile.cases) {
    rejectIdentitySeparator('case label', mobileCase.label);
    rejectIdentitySeparator('case key', mobileCase.key);
  ...
  for (const device of config.mobile.devices) {
    rejectIdentitySeparator('device key', device.key);
  ...
  // A per-case udid/serial endpoint override is incompatible with a device
  // matrix fan-out: the artifact identity becomes per-device
  // (label__device__key) while every row would capture from the case's single
  // override endpoint — evidence labeled 'iphone16' could have been captured
  // elsewhere. Fail loudly at validation time; the endpoint belongs on the
  // device rows themselves (a no-matrix config keeps the legacy override
  // chain — captureAllMobile resolves device → case → config).
  if (config.mobile.devices.length > 0) {
    for (const mobileCase of config.mobile.cases) {
      const fanOut = mobileCase.devices?.length ? mobileCase.devices.length : config.mobile.devices.length;
      if (fanOut > 0 && (mobileCase.udid || mobileCase.serial)) {
        throw new TypeError(
          `Mobile case "${mobileCase.key}" sets a per-case udid/serial endpoint override but the config declares ` +
          `${config.mobile.devices.length} device(s) and this case fans out to ${fanOut} of them; per-case endpoint ` +
          `overrides are incompatible with device matrix cases — move the endpoint to the device rows (mobile.devices).`
        );
      }
    }
  }
