...
export function normalizeConfig(input = {}, configPath = path.resolve('vision-loop.config.json')) {
  ...
  return {
    ...
    capture: {
      // Capture driver selector: playwright (web, default) or a mobile driver
      // (ios-sim via xcrun simctl; android is wired in a follow-up task).
      // Unknown values fall back to playwright so web behavior never changes
      // because of a typo in a mobile-only field.
    ...
    // Mobile capture matrix — only consumed when capture.type is ios-sim (or,
    // in a follow-up task, android). Cases ride through the web artifact
    // layout with identity { routeName: label, viewportName: 'mobile',
    // stateName: key } (see lib/mobile-capture-engine.mjs). Per-case
