...
export function validateVerdictRecord(parsed) {
  ...
  // Value-level checks: mode/severity enums, non-empty rule and judged_by,
  // expected+observed present per finding, ref fields string-or-null.
  // schemas/vision-judgment.schema.json encodes the same bounds
  // (rule minLength 1, judged_by pattern .*\S.*) so contract and runtime agree.
  const MODES = ['metrics', 'model', 'human'];
  if (!MODES.includes(parsed.mode)) {
    throw new TypeError(`mode must be ${MODES.join('|')}`);
  }
  if (parsed.judged_by.trim() === '') { // typeof string guaranteed above
