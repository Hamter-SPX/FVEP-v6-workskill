...
export async function captureAllMobile(config, { mode = 'current', filters = {}, exec, sleep } = {}) {
  ...
  if (platform === 'android') {
    // Task 4 wires captureAndroidScreenshot (adb screencap) here. Until then
    // this branch fails loudly instead of silently falling back to the simctl
    // driver, which would produce screenshots of the wrong device.
    throw new TypeError('captureAllMobile: capture.type "android" is Task 4 pending (captureAndroidScreenshot is not implemented yet)');
  }
