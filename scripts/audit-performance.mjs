...
const HELP = `Usage: node scripts/audit-performance.mjs [options]\n  -c, --config <path>\n      --base-url <url>\n      --headed\n      --route/--viewport/--state/--case <value>\n`;
try {
  ...
  if (args.help) printHelp(HELP); else {
    ...
    const results = await auditPerformanceAll(config, { baseUrl: args['base-url'], headed: args.headed, filters });
    const failed = results.filter((item) => !item.ok).length; process.stdout.write(`Performance cases: ${results.length}; failed: ${failed}; report: ${results.reportPath ?? 'disabled'}\n`); if (failed) process.exitCode = 1;
  }
